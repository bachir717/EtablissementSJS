...
--
-- Hôte : mysql
-- Généré le : jeu. 18 fév. 2021 à 09:53
...
CREATE PROCEDURE `deleteNote` (IN `n_id_notes` INT, OUT `erreur` INT)  BEGIN
    START TRANSACTION;
...
CREATE PROCEDURE `InsertNote` (IN `n_id_etudiant` INT, IN `n_id_matiere` INT, IN `n_note` INT, IN `n_coeif` INT, OUT `erreur` INT)  BEGIN
...
CREATE PROCEDURE `updateNote` (IN `n_id_notes` INT, IN `n_id_etudiant` INT, IN `n_id_matiere` INT, IN `n_note` INT, IN `n_coeif` INT, OUT `erreur` INT)  BEGIN
